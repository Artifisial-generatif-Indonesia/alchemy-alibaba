...
class StatefulACKClient extends ACKClient {
  ...
  override async describeClusterDetail(
    ...
    const cluster =
      this.cluster?.clusterId === clusterId ? this.cluster : undefined;
  ...
  override async createCluster(
    ...
    this.cluster = new ACK.DescribeClusterDetailResponseBody({
      ...
      clusterSpec: request.clusterSpec,
  ...
  override async modifyCluster(
    ...
    if (this.cluster !== undefined) {
      this.cluster = new ACK.DescribeClusterDetailResponseBody({
        ...this.cluster,
        clusterSpec: request.clusterSpec ?? this.cluster.clusterSpec,
        deletionProtection:
  ...
  override async createClusterNodePool(
    ...
    this.nodePool = new ACK.DescribeClusterNodePoolDetailResponseBody({
      ...
      scalingGroup:
        new ACK.DescribeClusterNodePoolDetailResponseBodyScalingGroup({
          ...
          instanceTypes: request.scalingGroup?.instanceTypes,
    ...
    return new ACK.CreateClusterNodePoolResponse({
      statusCode: 200,
      body: new ACK.CreateClusterNodePoolResponseBody({
        nodepoolId: "np-test",
      }),
    });
  }
      ...
      this.nodePool = new ACK.DescribeClusterNodePoolDetailResponseBody({
        ...
        scalingGroup:
          new ACK.DescribeClusterNodePoolDetailResponseBodyScalingGroup({
            ...
            instanceTypes:
              request.scalingGroup?.instanceTypes ??
              this.nodePool.scalingGroup?.instanceTypes,
            desiredSize:
...
describe("ACK provider lifecycles", () => {
  it("rejects ambiguous names and never switches a saved cluster identity to a namesake", async () => {
    const fake = new StatefulACKClient();
    fake.cluster = new ACK.DescribeClusterDetailResponseBody({
      clusterId: "c-namesake",
      name: "example-test",
      state: "running",
    });
    fake.decoyClusters = [{ clusterId: "c-second", name: "example-test" }];
    const layer = ManagedClusterProvider().pipe(
      Layer.provide(providerLayer(fake)),
    );
    const base = resourceBase("identity");
    const olds = { name: "example-test" } as never;
    await expect(
      Effect.runPromise(
        Effect.gen(function* () {
          return yield* (yield* ManagedCluster.Provider).read!({
            ...base,
            olds,
            output: undefined,
          });
    ...
    ).rejects.toMatchObject({ operation: "ObserveIdentity" });
    const inventories = fake.clusterListRequests.length;
      ...
      Effect.gen(function* () {
        return yield* (yield* ManagedCluster.Provider).read!({
          ...
          output: {
            ...
            name: "example-test",
            connection: {
              auth: { kind: "alibaba-ack", clusterId: "c-missing" },
            },
            state: "running",
            vswitchIds: [],
            deletionProtection: false,
    ...
    expect(fake.clusterListRequests).toHaveLength(inventories);
    expect(
      fake.clusterCreates + fake.clusterModifies + fake.clusterDeletes,
    ).toBe(0);
  });
  ...
  it("creates, updates, and deletes a cluster through transient teardown failures", async () => {
    ...
    const changed = {
      ...
      kubernetesVersion: "1.31.0-aliyun.1",
  ...
  it("creates, updates, and deletes a node pool after a transient failure", async () => {
    ...
    const changed = {
      ...initial,
      scalingGroup: { ...initial.scalingGroup, desiredSize: 2 },
      tags: { environment: "test" },
    };
    const program = Effect.gen(function* () {
      const provider = yield* NodePool.Provider;
  ...
  it("recovers a cluster orphaned by an interrupted create by paginating the region inventory", async () => {
    ...
    fake.cluster = new ACK.DescribeClusterDetailResponseBody({
      clusterId: "c-orphan",
      tags: [
        { key: "alchemy::stack", value: "provider-tests" },
        { key: "alchemy::stage", value: "test" },
        { key: "alchemy::id", value: "cluster-orphan" },
      ],
      name: "example-test",
      state: "running",
      clusterType: "ManagedKubernetes",
    ...
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        yield* provider.delete({
          ...
          output: {
            clusterId: "c-test",
            connection: { auth: { kind: "alibaba-ack", clusterId: "c-test" } },
            name: "example-test",
            state: "deleting",
            vswitchIds: ["vsw-test"],
            deletionProtection: false,
